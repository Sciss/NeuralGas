...
public class DemoGNG extends JApplet {
    ...
    class ALSlider implements ChangeListener {
        ...
        public void stateChanged(ChangeEvent e) {
            ...
            if (true/*!source.getValueIsAdjusting()*/) {
                panel.delay = (50-(int)source.getValue())*10;
            }
        }
    }
    // CheckBox Listener
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            if (evt.getSource() instanceof JButton) {
                ...
                // Restart
                    ...
                    compute.getRNG().setSeed(0L);   // XXX TODO TEST
                    reset();
                    start();
                }
            }
    ...
    public void prepareAlgo(Algorithm aNew){
        ...
        if (aNew.equals(Algorithm.GNG) || aNew.equals(Algorithm.GNGU)) {
            ...
            compute.algorithm = Algorithm.GNG;      // XXX TODO ???
...
//			edgefNGCHL_choice.setSelectedIndex(0);
                ...
                aNew.equals(Algorithm.LBGU))
        {
            ...
            // Generate some nodes
            int z = (int) (compute.numDiscreteSignals * compute.random());
            for (i = 0; i < compute.maxNodes; i++)
                compute.addNode(
                        Math.round(compute.discreteSignalsX[(z+i)%compute.numDiscreteSignals]),
...
//					}
                ...
                // max. edges
                    ...
                    compute.maxEdgeAge = maxEdgeAgeGNG_Ai[delEdgeGNG_choice.getSelectedIndex()];
                }
                // epsilon HCL
                ...
                // beta GNG
                    ...
                    compute.setBetaGNG(betaGNG_Af[betaGNG_choice.getSelectedIndex()]);
...
//                    compute.decayFactorUtility = 1.0f - betaGNG_Af[betaGNG_choice.getSelectedIndex()];
     ...
     */
    protected final static String ERROR_GRAPH   = " error graph";
    /**
     ...
     *  To add or delete values to the choice, only this array must be changed.
     */
    protected float epsilonGNG2_Af[] = {0.0006f, 0.0f, 0.0001f, 0.00025f, 0.0005f, 0.001f, 0.005f,
     ...
     *  To add or delete values to the choice, only this array must be changed.
     */
    protected float betaGNG_Af[] = {0.0005f, 0.0f, 0.000005f, 0.0000075f, 0.00001f, 0.0000125f, 0.000015f, 0.00002f, 0.00005f, 0.0001f,
    ...
    protected float utilityGNG_Af[] =  {3.0f, 1.0f, 1.5f, 2.0f, 2.5f,
            ...
            7.0f, 7.5f, 8.0f, 8.5f, 9.0f, 9.5f, 10.0f, 12.0f, 14.0f, 15.0f, 16.0f, 17.0f, 18.0f, 20.0f};
    /**
...
//        ComputeGNG.paintCounter = 0;
    ...
    void createGUI() {
        ...
        errorGraph_cb = new MyCheckBox(ERROR_GRAPH, panel.errorGraphB);
...
//				stepSize_choice.setSelectedIndex(3);
    ...
    public PD selectRandomPD() {
        Random random = compute.getRNG(); // new Random();
