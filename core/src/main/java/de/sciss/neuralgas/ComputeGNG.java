...
public class ComputeGNG {
     ...
     */
    public GridNodeGNG grid[][] = new GridNodeGNG[MAX_GRID_X][MAX_GRID_Y];

    /**
     * The array of the last computed numSignals (x-coordinate).
     */
    public float lastSignalsX[] = new float[MAX_STEP_SIZE];

    /**
     * The array of the last computed numSignals (y-coordinate).
     */
    public float lastSignalsY[] = new float[MAX_STEP_SIZE];

    /**
     * The array of the discrete numSignals (x-coordinate).
     */
    public float discreteSignalsX[] = new float[MAX_DISCRETE_SIGNALS];

    /**
     * The array of the discrete numSignals (y-coordinate).
     */
    public float discreteSignalsY[] = new float[MAX_DISCRETE_SIGNALS];
     ...
     */
    public int numDiscreteSignals = 500;
    ...
    public final double random() { return _random.nextDouble(); }

    public final Random getRNG() { return _random; }

    /**
     * The flag for entering the fine-tuning phase (GG).
     */
    public boolean fineTuningB = false;
     ...
     * close GG to a torus
     */
    public boolean torusGGB = false;

    /**
     * close SOM to a torus
     */
    public boolean torusSOMB = false;
     ...
     */
    public boolean LBG_U_B = false;

    /**
     * The flag for end of calculation (LBG)
     */
    public boolean readyLBG_B = false;

    /**
     * The selected distribution.
     *  This variable can be set by the user.
     */
    public PD pd = PD.Rectangle;

    /**
     * The selected algorithm.
     */
    public Algorithm algorithm = Algorithm.NG;

    /**
     * The current maximum number to delete an old edge (GNG,NGwCHL).
     *  This variable can be set by the user.
     */
    public int maxEdgeAge = 88;
     ...
     *  This variable can be set by the user.
     */
    public float epsilonGNG = 0.1f;

    /**
     * The value epsilon for the GNG algorithm (second).
     *  This variable can be set by the user.
     */
    public float epsilonGNG2 = 0.001f;
     ...
     *  This variable can be set by the user.
     */
    private float betaGNG = 0.0005f;

    public void setBetaGNG(float value) {
        betaGNG             = value;
        decayFactorUtility  = 1.0f - betaGNG;
        decayFactor         = 1.0f - betaGNG;
    }
     ...
     * Will be recalculated in setBetaGNG
     */
    private float decayFactorUtility = 1.0f - betaGNG;

    /*
     * The factor to forget old values.
     * Will be recalculated in setBetaGNG
     */
    private float decayFactor = 1.0f - betaGNG;

    /**
     * The value lambda initial for the NG,NGwCHL,GG algorithms.
     *  This variable can be set by the user.
     */
    public float l_i = 30.0f;

    /**
     * The value lambda final for the NG,NGwCHL,GG algorithms.
     *  This variable can be set by the user.
     */
    public float l_f = 0.01f;
     ...
     */
    public float errorBestLBG_U = Float.MAX_VALUE;
     ...
     */
    public int addNode(Dimension d) {
        ...
        if (rndInitB) {
            ...
            n.x = (float) (10 + (d.width -20) * random()); // Math.random()
            n.y = (float) (10 + (d.height-20) * random()); // Math.random()
        } else {
     ...
     * @param d          The dimension of the initial drawing area
     */
    public void initGrid(int width, int height, Dimension d) {
    ...
    public void rescaleDiscreteSignals(double x, double y) {
        ...
        for (k = 0; k < stepSize; k++) {
            ...
            if (!algorithm.isDiscrete()) { // neither LBG nor LBG-U
                ...
                for (i = 0 ; i < nNodes ; i++) {
                    ...
                    // Calculate node with minimum utility (GNG-U)
                ...
                case GNG:
                    ...
                    if ( (numSignals % lambdaGNG) == 0 ) {
                        if (!noNewNodesGNGB) {
                            ...
                            final int nodeIdx = insertNode(numError, maximumErrorNeighbor(numError));
//                            result.insertedSound = nodeIdx != -1;
                        }
                    }
